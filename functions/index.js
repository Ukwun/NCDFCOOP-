...
exports.notifySellerOnProductApproval = exports.releaseSellerEarningsOnDelivery = exports.deleteMyAccount = exports.claimMemberReward = exports.reviewSellerWithdrawal = exports.requestSellerWithdrawal = exports.initializeFlutterwavePayment = exports.stripeWebhook = exports.initializeStripeCheckout = exports.createMarketplaceOrder = exports.logProductView = exports.getRecommendedProducts = exports.onOrderPaymentConfirmed = exports.fulfillOrder = exports.calculateDailyAnalytics = exports.autoTriggerReorders = exports.autoPromoteMemberTier = exports.calculateLoyaltyPoints = exports.deactivateSellerProductOffer = exports.upsertSellerProductOffer = exports.reviewSellerProduct = exports.getAdminDashboardMetrics = exports.listPendingSellerProducts = exports.resolveMarketplaceIdentity = exports.provisionMarketplaceRole = void 0;
...
function requireTrustedApp(context) {
    if (!context.app) {
        throw new functions.https.HttpsError('failed-precondition', 'Requests must come from a verified CoopX app.');
    }
}
...
async function legacyRoleProfileExists(collection, uid, email) {
    if ((await db.collection(collection).doc(uid).get()).exists)
        return true;
...
async function queryExists(collection, field, value) {
    if (!value)
        return false;
 ...
 */
exports.resolveMarketplaceIdentity = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Sign in before resolving your marketplace identity.');
    }
    const uid = context.auth.uid;
    ...
    const [sellerProfile, wholesaleBuyer, coopMember, sellerCommerce] = await Promise.all([
        Promise.all([
    ...
    const declaredRoles = declaredValues
        ...
        .replace(/[^a-z0-9]/g, ''))
        .map((value) => {
        if (['seller', 'vendor', 'merchant'].includes(value))
            return 'seller';
...
function requireMarketplaceAdmin(context) {
    ...
    if (!context.auth || !privileged) {
        throw new functions.https.HttpsError('permission-denied', 'Only an authorized administrator can review seller products.');
    }
}
/** Returns real pending seller products to the moderation dashboard. */
exports.listPendingSellerProducts = functions.https.onCall(async (_data, context) => {
...
exports.getAdminDashboardMetrics = functions.https.onCall(async (_data, context) => {
    requireMarketplaceAdmin(context);
    let totalUsers = 0;
    let pageToken;
    do {
    ...
    const [pendingProducts, pendingWithdrawals, orders, activeProducts] = await Promise.all([
...
exports.reviewSellerProduct = functions.https.onCall(async (data, context) => {
    requireMarketplaceAdmin(context);
    ...
    if (!productId || !['approved', 'rejected'].includes(decision)) {
        throw new functions.https.HttpsError('invalid-argument', 'Choose a valid product and moderation decision.');
    }
    if (decision === 'rejected' && note.length < 3) {
        throw new functions.https.HttpsError('invalid-argument', 'Explain why this product is being rejected.');
    }
    ...
    if (!sellerSnapshot.exists || sellerSnapshot.data()?.status !== 'pending') {
        throw new functions.https.HttpsError('failed-precondition', 'This product is no longer pending review.');
    }
...
exports.upsertSellerProductOffer = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (!productId || title.length < 3 ||
        ...
        throw new functions.https.HttpsError('invalid-argument', 'Enter a valid offer, discount, audience, and date range.');
    }
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (ownerId !== context.auth.uid || product.status !== 'approved') {
            throw new functions.https.HttpsError('permission-denied', 'Only the owner can create an offer for an approved product.');
        }
...
exports.deactivateSellerProductOffer = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Sign in as a seller.');
    }
    const productId = String(data.productId || '').trim();
...
exports.requestSellerWithdrawal = functions.https.onCall(async (data, context) => {
    ...
    if (bankName.length < 2 || !/^\d{3,6}$/.test(bankCode) ||
        ...
        throw new functions.https.HttpsError('invalid-argument', 'Enter valid Nigerian bank and account details.');
    }
    let assignedAdminId = '';
    try {
        assignedAdminId =
        ...
        throw new functions.https.HttpsError('failed-precondition', 'Withdrawal review is temporarily unavailable.');
    }
    const earningsRef = db.collection('seller_earnings').doc(context.auth.uid);
    const payoutAccountRef = db.collection('seller_payout_accounts').doc(context.auth.uid);
...
exports.reviewSellerWithdrawal = functions.https.onCall(async (data, context) => {
    if (!context.auth ||
        String(context.auth.token.email || '').toLowerCase() !==
            PRIMARY_SUPER_ADMIN_EMAIL ||
        context.auth.token.email_verified !== true) {
        throw new functions.https.HttpsError('permission-denied', 'Only the verified super-admin account can review withdrawals.');
    }
    ...
    if (!withdrawalId || !['approved', 'rejected'].includes(decision)) {
        throw new functions.https.HttpsError('invalid-argument', 'Choose an approval decision.');
    }
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!snapshot.exists || snapshot.data()?.status !== 'pending') {
            throw new functions.https.HttpsError('failed-precondition', 'This request has already been reviewed or no longer exists.');
        }
...
exports.notifySellerOnProductApproval = functions.firestore
    ...
    const batch = db.batch();
    batch.set(db.collection('notifications').doc(sellerId).collection('items').doc(notificationId), {
