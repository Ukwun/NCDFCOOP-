...
app.post('/processRefund', async (req, res) => {
  try {
    ...
    const refundResponse = await axios.post(
      `https://api.flutterwave.com/v3/transactions/${txRef}/refund`,
      { amount },
      {
        headers: {
          Authorization: `Bearer ${FLUTTERWAVE_SECRET_KEY}`,
 ...
 * 4. Log activity
 */
export const fulfillOrder = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    try {
      ...
      const deductionResults = await db.runTransaction(async transaction => {
        ...
        if (latestData.fulfillmentStatus === 'created_shipment') return null;
        if (latestData.paymentStatus !== 'paid') throw new Error('Payment is not confirmed.');
        ...
        if (requested.size === 0) throw new Error('Order has no fulfillable items.');
        ...
        productEntries.forEach((entry, index) => {
          ...
          if (!productSnapshot.exists) throw new Error(`Product ${entry.productId} is unavailable.`);
      ...
      if (deductionResults === null) return null;
 ...
 */
export const onOrderPaymentConfirmed = functions.firestore
  .document('orders/{orderId}')
  .onUpdate(async (change, context) => {
    const before = change.before.data();
    const after = change.after.data();
 ...
 */
export const getRecommendedProducts = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
    ...
    try {
      ...
      viewedSnapshot.forEach((doc) => {
        ...
        if (category) viewedCategories.add(category);
      });
 ...
 */
export const logProductView = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
...
export const createMarketplaceOrder = functions.https.onCall(async (data, context) => {
  ...
  if (!addressId || !paymentMethodId) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Delivery address and payment method are required.',
    );
  }
  const uid = context.auth.uid;
  const [userSnapshot, addressSnapshot, paymentMethodSnapshot, configSnapshot] =
    await Promise.all([
  ...
  if (!addressSnapshot.exists || !paymentMethodSnapshot.exists) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'The selected delivery or payment details are unavailable.',
    );
  }
...
export const initializeStripeCheckout = functions
...
export const releaseSellerEarningsOnDelivery = functions.firestore
  .document('orders/{orderId}')
  .onUpdate(async (change, context) => {
    const before = change.before.data();
    const after = change.after.data();
    ...
    for (const settlementDoc of settlements.docs) {
      await db.runTransaction(async (transaction) => {
        ...
        if (!latest.exists || latest.data()?.status !== 'pending_fulfillment') return;
